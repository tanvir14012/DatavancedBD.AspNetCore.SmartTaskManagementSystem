...
`VITE_API_BASE_URL` is validated at startup and accepts an absolute URL or `/services`.
When omitted, it defaults to `/services`; Vite proxies this path to the local HTTP API
at `http://localhost:5049`. Docker builds explicitly use `/services`, which the local
SaaS Nginx configuration routes to the matching company's API.
The optional environment file above selects direct HTTPS API access instead:
...
To run all local SaaS companies with React, run
`./deploy/local/Start-LocalSaas.ps1 -FrontendClient React` from the repository root.
See [the local SaaS guide](../../deploy/local/README.md) for the persistent
`frontendClient` setting, company URLs, and login instructions.
